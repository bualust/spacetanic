...
def main():
    ...
    train_data = data_manipulation("train.csv")
...
def data_manipulation(filename):
    ...
    feat_type_dict = columns_types(data)
    data = remove_nan(data, feat_type_dict)
    if DO_FEATS_PLOT:
        plot_variables(data, feat_type_dict, "before_enc")
    data = encode_data(data, feat_type_dict)
    if DO_FEATS_PLOT:
...
def run_XGBoost(X_train, X_test, Y_train, Y_test):
    ...
    XGB_roc_auc_curve = roc_auc_score(Y_test, y_pred_XGB)
    print(f"Model accuracy is {XGB_accuracy}")
    print(f"ROC auc is {XGB_roc_auc_curve}")
